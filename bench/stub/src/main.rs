...
async fn main() {
    ...
    let listener = match TcpListener::bind(("127.0.0.1", port)).await {
        ...
        Err(error) => {
            eprintln!("benchmark stub: failed to bind 127.0.0.1:{port}: {error}");
            std::process::exit(1);
        }
    };
    let address: SocketAddr = match listener.local_addr() {
        Ok(address) => address,
        Err(error) => {
            eprintln!("benchmark stub: failed to inspect listener: {error}");
            std::process::exit(1);
        }
    };
    ...
    if let Err(error) = axum::serve(listener, app)
        ...
        .await
    {
        eprintln!("benchmark stub: server error: {error}");
        std::process::exit(1);
    }
}
