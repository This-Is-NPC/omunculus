defmodule Omunculus.Model.OpenAIResponsesTest do
  use ExUnit.Case

  @moduletag :cargo

  alias Omunculus.{ExecutionPolicyFixtures, Fixtures}
  alias Omunculus.Model.OpenAIResponses
  ...
  test "tool_rounds: 1 calls the tool exactly once and returns the final content", %{
    ...
    model = OpenAIResponses.new(spec(base_url))
    {agent, call} = recorder()
  ...
  test "an unreachable base_url yields {:error, {:openai, _}}" do
    model = OpenAIResponses.new(spec("http://127.0.0.1:1"))
  ...
  test "records all model turns and executes tools", %{
    ...
    assert Jason.decode!(first.body)["output"] != []
    assert Jason.decode!(last.body)["output"] != []
  end
